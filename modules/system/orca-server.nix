...
let
  ...
  updateSettings = import ../../packages/update-orca-settings.nix { inherit pkgs; };
  version = "1.4.193";

  sources = {
    aarch64-linux = {
      artifact = "orca-linux-arm64.AppImage";
      hash = "sha256-/yw7e2UcTH2I3w38iNLVIMuo4OgUsiMGa294tFvejks=";
    };
    x86_64-linux = {
  ...
  src = pkgs.fetchurl {
    url = "https://github.com/stablyai/orca/releases/download/v${version}/${source.artifact}";
    inherit (source) hash;
  };
  ...
  orcaApp = pkgs.appimageTools.wrapType2 {
    ...
    # The FHS wrapper normally replaces /etc. Keep the host's tracked
  ...
  serverLauncher = pkgs.writeShellScript "orca-server" ''
    ...
    ${lib.getExe updateSettings} \
      "$XDG_CONFIG_HOME/orca/profiles/local-default/orca-data.json" \
    ...
    exec ${lib.getExe' pkgs.dbus "dbus-run-session"} -- \
      ${lib.getExe orcaCli} serve \
        --port 6768 \
        --pairing-address "''${addresses[0]}" ${lib.optionalString cfg.mobilePairing "--mobile-pairing"}
  '';
in
{
  options.services.orcaServer = {
    enable = lib.mkEnableOption "the tailnet-accessible Orca remote server";
    user = lib.mkOption {
      type = lib.types.str;
      description = "Existing local user that owns Orca's state and processes.";
    };
    mobilePairing = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = "Enable Orca's mobile pairing mode.";
    };
    extraPath = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ ];
      description = "Additional executable directories visible to Orca.";
    };
  };
      ...
      environment = {
        ...
        PATH = lib.mkForce (
          lib.concatStringsSep ":" (
            [ "/etc/profiles/per-user/${cfg.user}/bin" ] ++ cfg.extraPath ++ [ "/run/current-system/sw/bin" ]
          )
        );
      };
